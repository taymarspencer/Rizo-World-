...
with sync_playwright() as p:
    ...
    capture(browser,'01-phone-short-planning',(320,568),'grove', '''()=>{RizoRuntimeQA.defenseSetCashForQA(9999);RizoRuntimeQA.defensePlaceNextForQA();RizoRuntimeQA.defenseSetRunForQA({wave:123,phase:"planning"});}''')
    capture(browser,'02-phone-combat-packet',(360,640),'grove', '''()=>{RizoRuntimeQA.defenseSetCashForQA(9999);for(let i=0;i<5;i++)RizoRuntimeQA.defensePlaceNextForQA();RizoRuntimeQA.defenseLoadQueueForQA(32,"puff",18,1);RizoRuntimeQA.defenseSpawnForQA("shell",.45);RizoRuntimeQA.defenseSpawnForQA("fleet",.62);}''',900)
    capture(browser,'03-phone-standard-combat',(390,844),'ember', '''()=>{RizoRuntimeQA.defenseSetCashForQA(9999);for(let i=0;i<6;i++)RizoRuntimeQA.defensePlaceNextForQA();RizoRuntimeQA.defenseLoadQueueForQA(36,"fleet",27,2);RizoRuntimeQA.defenseSpawnForQA("shell",.34);RizoRuntimeQA.defenseSpawnForQA("split",.55);}''',900)
    capture(browser,'04-phone-tower-panel-world-perk',(430,932),'ember', '''()=>{RizoRuntimeQA.defenseSetCashForQA(9999);RizoRuntimeQA.defenseSetPetVariantForQA("ember");for(let i=0;i<4;i++)RizoRuntimeQA.defensePlaceNextForQA();RizoRuntimeQA.defenseShowTowerPanelForQA();}''')
    capture(browser,'05-landscape-command-wings',(844,390),'storm', '''()=>{RizoRuntimeQA.defenseSetCashForQA(9999);for(let i=0;i<6;i++)RizoRuntimeQA.defensePlaceNextForQA();RizoRuntimeQA.defenseLoadQueueForQA(30,"fleet",35,2);RizoRuntimeQA.defenseSpawnForQA("boss",.35);}''',900)
    capture(browser,'06-tablet-portrait-field-menu',(768,1024),'blizzard', '''()=>{RizoRuntimeQA.defenseSetCashForQA(9999);for(let i=0;i<6;i++)RizoRuntimeQA.defensePlaceNextForQA();RizoRuntimeQA.defenseToggleFieldMenuForQA(true);}''')
    ...
    capture(browser,'10-world-lobby-opening-perk',(390,844),'grove', '''()=>{RizoRuntimeQA.defenseShowLobbyForQA("ember");}''')
    browser.close()
