...
(() => {
  ...
  */
  ...
  function defenseDurabilityScale(wave,{boss=false}={}){const w=Math.max(1,Number(wave)||1),late=1+Math.max(0,w-30)*.012+Math.max(0,w-60)*.018+Math.max(0,w-90)*.020;return late*(boss?(w<=10?.88:1.72):1);}
  function defenseCanonicalEnemySnapshot(raw,currentWave,map,index=0,preciseArmor=false){
    if(!raw||typeof raw!=="object")return null;
  ...
  function normalizeDefenseCheckpoint(raw){
    if(!raw||typeof raw!=="object")return null;
    const version=Number(raw.checkpointVersion)||1;if(![1,2,3,4,5,6,7,DEFENSE_CHECKPOINT_VERSION].includes(version))return null;
    ...
    const enemies=trusted?(Array.isArray(raw.enemies)?raw.enemies:[]).slice(0,DEFENSE_LIMITS.MAX_CHECKPOINT_ENEMIES).map((enemy,index)=>defenseCanonicalEnemySnapshot(enemy,currentWave,map,index,version>=8)).filter(Boolean):[];
  ...
  function recordDefenseRun(snapshot){
    ...
    const leaders=[...petTotals.values()].sort((a,b)=>b.damage-a.damage||b.kills-a.kills),persistentLeaders=leaders.filter(row=>!row.petId.startsWith("defense-crew-")),top=persistentLeaders[0]||{petId:state.pet.id||"",name:state.pet.name,variant:state.pet.variant||"classic",damage:0};state.scores.defenseMastery||={};
  ...
  function defenseRoster(){
    ...
    const playable=rows.filter(row=>row.pet?.alive!==false&&row.pet?.stage!=="egg");
    // Trail crew are Defense characters, never adoptions or care-state entries.
    // Stable IDs let the existing signed checkpoint registry restore them.
    if(playable.length)for(const [variant,name] of [["violet","SHORT CIRCUIT"],["frost","COLD SHOULDER"],["obsidian","DEAD WEIGHT"]]){
      const pet={...state.pet,id:`defense-crew-${variant}`,name,variant,hiddenVariant:variant,stage:"kid",alive:true,defenseGuest:true,accessory:null,skills:{power:0,speed:0,instinct:0,stamina:0,luck:0}};
      playable.push({pet,source:"house",rosterIndex:-1});
    }
    return playable;
  }
  ...
  function defenseUpgradeMove(tower){const v=tower.pet.variant||tower.pet.hiddenVariant||"classic";if(tower.upgrade===0)return{classic:"Every third shot hits 65% harder",violet:"Chain reaches a fourth target",frost:"Chill splashes into nearby threats",obsidian:"Every impact strips armor"}[v]||"Stronger hits and wider reach";if(tower.upgrade===1)return"Choose POWER impacts or CONTROL pulses + field power";return tower.doctrine==="control"?"Stronger control and wider coverage":"Harder charged strikes";}
  ...
  function defenseRosterMarkup(){
    ...
    return defenseRoster().map(row=>{
      ...
      const instruction=placementLocked?"RESUME TO BUILD":selected?"TAP MAP OR DRAG":uniqueBlocked?"ONE PER CONTRACT":row.source==="active"&&!copies?"FREE DEPLOY":d.towers.length>=d.maxTowers?`FIELD FULL • ${d.maxTowers}`:`${cost} COINS`;
      const role=defenseTowerStats(row.pet).profile.label;
      const benchInstruction=placementLocked?"PAUSED":selected?"TAP / DRAG":uniqueBlocked?"ONE PER RUN":d.towers.length>=d.maxTowers?"FIELD FULL":d.cash<cost?"NEED CASH":role;
  ...
  function defenseWavePlan(wave){
    ...
    const bossEntry=DefenseCore.flattenPackets(plan.packets).find(entry=>typeof entry==="object"&&entry?.type==="boss");if(bossEntry){const boss=defenseBossForWave(wave);d.waveAnnouncement={title:`${boss.name} • BOSS WAVE`,copy:wave===10?plan.announcement.copy:boss.hint};}
    return plan;
  }
  ...
  function fireDefenseTower(tower,target,stats){
    ...
    if(tower.node){const field=mini.defense,dx=(target.x-tower.x)*(field.renderWidth||390),dy=(target.y-tower.y)*(field.renderHeight||500),length=Math.hypot(dx,dy)||1,nx=dx/length,ny=dy/length;tower.node.style.setProperty("--kick-x",`${-nx*5}px`);tower.node.style.setProperty("--kick-y",`${-ny*5}px`);tower.node.style.setProperty("--soft-kick-x",`${-nx*2.5}px`);tower.node.style.setProperty("--soft-kick-y",`${-ny*2.5}px`);tower.node.style.setProperty("--heavy-kick-x",`${-nx*6.25}px`);tower.node.style.setProperty("--heavy-kick-y",`${-ny*6.25}px`);tower.node.style.setProperty("--lunge-x",`${nx*4}px`);tower.node.style.setProperty("--lunge-y",`${ny*4}px`);tower.node.style.setProperty("--heavy-lunge-x",`${nx*4.8}px`);tower.node.style.setProperty("--heavy-lunge-y",`${ny*4.8}px`);tower.node.style.setProperty("--aim-angle",`${Math.atan2(dy,dx)*180/Math.PI}deg`);}
    ...
    if(variant==="classic"&&tower.upgrade>=1&&shotNumber%3===0)damage*=1.65;
  ...
  queueMiniTimeout(()=>releaseDefenseEnemyNode(enemy),enemy.bossId?280:185);removeDefenseRuntimeItem(d.enemies,enemy);removeDefenseRuntimeItem(mini.entities,enemy);if(enemy.type==="split"){d.waveTotal+=2;queueDefenseChildSpawn("puff",{progress:enemy.progress,delay:.08});queueDefenseChildSpawn("fleet",{progress:Math.max(0,enemy.progress-.018),delay:.16});}if(enemy.bossId){d.bossesBeaten.push(enemy.bossId);d.bossesDefeated=(d.bossesDefeated||0)+1;showDefenseCinematicMoment("clear",{title:"BOSS POPPED",copy:(DEFENSE_BOSSES.find(item=>item.id===enemy.bossId)?.name||"BOSS")+" IS OFF THE TRAIL",priority:7,duration:1100});}if(enemy.bossId||defenseRealNow(d)-d.lastPopSfxAt>.065){d.lastPopSfxAt=defenseRealNow(d);sfx(enemy.bossId?"legendary":"spark");}}
  ...
  function updateDefenseTowers(dt){
    const d=mini.defense,time=defenseNow(),realTime=time,budget=DEFENSE_BUDGETS.normal;let goldenAwakened=0;
  ...
  function updateDefenseEnemies(dt){
    ...
    for(let index=d.enemies.length-1;index>=0;index-=1){
      ...
      const rooted=enemy.rootUntil>time,slow=enemy.slowUntil>time?1-enemy.slow:1,stormPulse=enemy.stormPulse?(((time+enemy.phaseOffset)%4.8)<1.15?.65:((time+enemy.phaseOffset)%4.8)<2.2?1.85:1):1,apexPulse=enemy.bossId==="apex"?(enemy.apexSurgeUntil>time?1.9:.82):1,mapPulse=d.stormWeatherUntil>time?1.32:1;
